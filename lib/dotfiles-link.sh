...
#
# Sourced by bootstrap.sh and rebuild.sh. Both used to carry the same bare
# `ln -sfn "$DIR" ~/.dotfiles`, which silently does the wrong thing when
# ~/.dotfiles already exists as a real directory: -n only guards against an
# existing *symlink*, so against a directory ln happily creates the link
# *inside* it and exits 0. Keeping the logic here means the two scripts cannot
# drift apart again.
#
